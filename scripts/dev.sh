...
COMPOSE_FILE="infra/compose/podman-compose.dev.yml"
...
generate_and_serve_docs() {
    ...
    if [ -f coverage.json ]; then
        ...
        python3 scripts/generate_reports.py --type coverage --input . --output docs/reports/coverage-report.md
    fi
    ...
    if [ -f junit.xml ]; then
        ...
        python3 scripts/generate_reports.py --type test --input test-results --output docs/reports/test-report.md
    fi

    # Run security scan
    log_info "Running security scan..."
    podman-compose -f "$COMPOSE_FILE" exec -T api bash -c \
        "pip install bandit && bandit -r backend/ -f json -o bandit-report.json || true" || true
    ...
    if [ -f bandit-report.json ]; then
        ...
        python3 scripts/generate_reports.py --type security --input security-results --output docs/reports/security-report.md
    fi
    ...
    if [ -f licenses.json ]; then
        ...
        python3 scripts/generate_reports.py --type license --input license-results --output docs/reports/license-report.md
    fi

    # Generate CI/CD report
    log_info "Generating CI/CD pipeline report..."
    python3 scripts/generate_reports.py --type cicd --output docs/reports/ci-cd.md
