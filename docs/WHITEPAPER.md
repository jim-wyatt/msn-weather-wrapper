...
Frontend data sources such as `frontend/app/data/cities.ts` supply autocomplete datasets, while `frontend/tests/e2e/` houses Playwright specs that mirror user journeys. Backend tests in `tests/` validate caching rules, security controls, and integration workflows. The `scripts/` directory contains report generators, SBOM scripts, and deployment helpers that round out the operational toolchain.
