...
Developer onboarding is documented in `docs/DEVELOPMENT.md` and `docs/CONTAINER_DEV_SETUP.md`, covering both containerized and local flows. The `dev.sh` script codifies common tasks (rebuild, clean, logs, docs) to avoid one-off commands. Pre-commit hooks enforce formatting, linting, and type checks before code reaches CI, aligning local and remote gates. The project keeps feedback fast with hot reload in Vite, FastAPI reload during local API runs, and cached Docker layers in CI.
...
Security is shifted left with Bandit and Semgrep in CI, augmented by mypy and Ruff to prevent common defect classes. Dependency risk is managed through Trivy and Grype scans, while SBOMs and license reports document supply-chain posture. At runtime, Nginx enforces reverse-proxy controls, the FastAPI app applies validation and rate limits, and strict Pydantic models guard the API surface. GitHub Actions jobs run with least-privilege tokens and isolated environments to reduce blast radius.
...
Frontend data sources such as `frontend/src/data/cities.ts` supply autocomplete datasets, while `frontend/tests/e2e/` houses Playwright specs that mirror user journeys. Backend tests in `tests/` validate caching rules, security controls, and integration workflows. The `scripts/` directory contains report generators, SBOM scripts, and deployment helpers that round out the operational toolchain.
